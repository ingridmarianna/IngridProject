...
- (FGTranslator *)translator {
    ...
    FGTranslator *translator = [[FGTranslator alloc] initWithBingAzureClientId:@"IngridEmoticonApp" secret:@"DVRZluCXIICgxQwtcxpdn4JIh6W2XF5MnespQt0oiOw="];
...
- (IBAction)switchKeyboard:(UIButton *)sender {
    ...
    if (self.translator_view.isFirstResponder) {
        if (self.translator_view.emoticonsKeyboard) [self.translator_view switchToDefaultKeyboard];
        else [self.translator_view switchToEmoticonsKeyboard:[WUDemoKeyboardBuilder sharedEmoticonsKeyboard]];
    }else{
        [self.translator_view switchToEmoticonsKeyboard:[WUDemoKeyboardBuilder sharedEmoticonsKeyboard]];
        [self.translator_view becomeFirstResponder];
    }
}


- (IBAction)translator_button:(id)sender {
    
    
  //  [SVProgressHUD show];
    
    [self.translator_view resignFirstResponder];
    
    [self.translator translateText:self.translator_view.text
   ...
   //          [SVProgressHUD dismiss];
         ...
         else
         {
            ...
            NSLog(@"translated from %@: %@", fromLanguage, translated);
             ...
             _resultView.text = [_resultView.text stringByAppendingString:translated];
